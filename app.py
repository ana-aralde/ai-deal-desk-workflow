...
def ai_permitted() -> bool:
    password_required = bool(secret("APP_PASSWORD"))
    authenticated = st.session_state.get("authenticated", not password_required)
    return authenticated or bool_secret("ALLOW_PUBLIC_AI", False)
...
def call_ai_analysis(request: Dict[str, Any], sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if not ai_permitted():
        raise RuntimeError("AI calls are disabled for unauthenticated public sessions.")

    max_calls = int_secret("MAX_AI_CALLS_PER_SESSION", 5)
...
def authenticate() -> None:
    ...
    if st.session_state.get("authenticated"):
        return
    st.subheader("Controlled demo access")
    ...
    st.info("The access code protects AI usage and reduces the risk of unauthorized API consumption.")
    st.stop()
...
def sidebar() -> None:
    with st.sidebar:
        ...
        st.write("**AI status**")
        if ai_available():
            st.success(f"Configured · {secret('OPENAI_MODEL', 'gpt-5.6-terra')}")
        else:
            st.warning("Not configured · rule engine still works")
        st.write(f"AI calls this session: {st.session_state.get('ai_calls', 0)} / {int_secret('MAX_AI_CALLS_PER_SESSION', 5)}")
        st.divider()
...
def ai_review_tab() -> None:
    ...
    if st.button("Run AI review", type="primary", disabled=not confirm):
        if not relevant:
    ...
    if result:
        c1, c2 = st.columns(2)
        c1.write("**AI review path:**", result.get("review_path"))
        c2.write("**AI risk level:**", result.get("risk_level"))
        st.write("**Summary**")
