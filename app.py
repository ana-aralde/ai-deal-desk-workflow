...
def dashboard_tab() -> None:
    ...
    snapshots = [governance_snapshot(r) for r in requests]
    enhanced = sum(1 for g in snapshots if g.get("effective_path") == "Enhanced")
    escalations = sum(1 for g in snapshots if g.get("governance_status") == "Escalation required")
    due_3 = 0
    overdue = 0
    for r in requests:
        try:
    ...
    c2.metric("Effective Enhanced", enhanced)
    c3.metric("AI escalations", escalations)
    c4.metric("Due ≤ 3 days", due_3)
    ...
    if blockers:
        with st.expander("Human resolution / exception record", expanded=not bool(override.get("approved"))):
            ...
            if st.button("Record human decision", key=f"record_override_{req_id}"):
                if not approver.strip() or not rationale.strip() or not attested:
                    st.error("Named approver, rationale and confirmation are required.")
                else:
                    req["human_override"] = {
                        "approved": True,
...
- **Human release gate** blocks release when AI surfaces unresolved High-risk issues, missing information or a triage discrepancy; any exception requires a named human decision and rationale.
