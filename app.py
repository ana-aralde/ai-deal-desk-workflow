...
# -----------------------------
# Source temporal governance
# -----------------------------
def source_temporal_status(source: Dict[str, Any], submission_deadline: Any) -> Dict[str, Any]:
    """
    Interpret review_date as NEXT review date.
    A source is within its review window when the submission deadline is on or before
    the next review date. It is potentially stale only when the deadline falls after it.
    """
    next_review_raw = source.get("review_date")
    deadline_raw = submission_deadline
    try:
        next_review = date.fromisoformat(str(next_review_raw))
        deadline = date.fromisoformat(str(deadline_raw))
    except Exception:
        return {
            "next_review_date": str(next_review_raw or ""),
            "temporal_status_for_submission": "Unknown",
            "temporal_rule": "Could not parse one or both dates; human validation required.",
        }

    if deadline <= next_review:
        status = "Within review window"
        rule = "Submission deadline is on or before the source's next review date."
    else:
        status = "Potentially stale at submission deadline"
        rule = "Submission deadline is after the source's next review date; freshness must be revalidated."

    return {
        "next_review_date": str(next_review),
        "temporal_status_for_submission": status,
        "temporal_rule": rule,
    }


def normalize_response_matrix(result: Dict[str, Any], request: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Normalize model output into a requirement-by-requirement response matrix."""
    requirements = request.get("requirements", []) or []
    req_by_id = {str(r.get("requirement_id")): r for r in requirements if r.get("requirement_id")}
    raw = result.get("response_matrix") or []

    # Backward/fallback support if the model returned draft_response as a dict keyed by requirement_id.
    if not raw and isinstance(result.get("draft_response"), dict):
        raw = [
            {
                "requirement_id": rid,
                "draft_response": draft,
                "source_ids": [],
                "suggested_owner": req_by_id.get(str(rid), {}).get("suggested_owner", "Deal Desk"),
                "review_status": "Human QA required",
                "notes": "Converted from draft_response fallback.",
            }
            for rid, draft in result.get("draft_response", {}).items()
        ]
    ...
    for item in raw if isinstance(raw, list) else []:
        ...
        normalized.append({
            "requirement_id": rid,
            "requirement": req.get("requirement", item.get("requirement", "")),
            "category": req.get("category", item.get("category", "")),
            "suggested_owner": item.get("suggested_owner") or req.get("suggested_owner", "Deal Desk"),
            "draft_response": item.get("draft_response", ""),
            "source_ids": ", ".join(str(x) for x in src_ids if x) or "—",
            "review_status": item.get("review_status") or "Human QA required",
            "notes": item.get("notes") or "",
        })
        seen.add(rid)

    # Ensure every extracted requirement is represented, even if the model omitted one.
    for req in requirements:
        ...
        normalized.append({
            ...
            "suggested_owner": req.get("suggested_owner", "Deal Desk"),
            "draft_response": "",
            "source_ids": "—",
            "review_status": "Evidence / draft missing",
            "notes": "The AI response did not return a row for this requirement; human follow-up required.",
        })
    return normalized
...
def call_ai_analysis(request: Dict[str, Any], sources: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for s in sources:
        ...
        total_redactions += n
        temporal = source_temporal_status(s, request.get("deadline"))
        source_payload.append({
            "source_id": s.get("source_id"),
            "title": s.get("title"),
            "owner": s.get("owner"),
            "next_review_date": temporal.get("next_review_date"),
            "temporal_status_for_submission": temporal.get("temporal_status_for_submission"),
            "temporal_rule": temporal.get("temporal_rule"),
            "content": clean,
        })
...
7. IMPORTANT DATE SEMANTICS: approved_sources.next_review_date means the NEXT scheduled review date, not the date the source was created or last approved. If the submission deadline is on or before next_review_date, do NOT flag the source as stale merely because next_review_date occurs after the deadline. If the deadline is after next_review_date, flag potential staleness and require human revalidation. Follow temporal_status_for_submission when supplied.
8. Build a requirement-by-requirement response_matrix for every extracted requirement. Each row must contain: requirement_id, draft_response, source_ids, suggested_owner, review_status, notes.
9. review_status must be one of: Ready for human QA, SME approval required, Evidence missing, Blocked.
10. Use "Ready for human QA" only when the draft is supported by approved sources and no specialist approval is still required. Use "SME approval required" where an approved source itself requires Legal, Privacy, Security/GRC, Finance, Product or another accountable owner to validate the statement. Use "Evidence missing" when approved evidence is insufficient. Use "Blocked" when the response should not proceed until a material issue is resolved.
11. Return valid JSON only, with these keys: summary, review_path, risk_level, suggested_owners, detected_flags, missing_information, source_ids_used, response_matrix, draft_response, qa_checks, human_approval_required.
12. draft_response may contain an overall customer-facing draft or short cover response, but response_matrix is the primary requirement-level work product.
13. review_path must be one of Simplified, Standard, Enhanced. risk_level must be Low, Medium, High.
14. human_approval_required must always be true.
""".strip()
...
def source_library_tab() -> None:
    ...
    if st.session_state["sources"]:
        sdf = pd.DataFrame(st.session_state["sources"]).copy()
        display_sdf = sdf[["source_id", "title", "owner", "status", "review_date"]].rename(
            columns={"review_date": "next_review_date"}
        )
...
def ai_review_tab() -> None:
    ...
    if result:
        ...
        matrix_rows = normalize_response_matrix(result, req)
        st.write("**RFP response matrix — NOT APPROVED**")
        if matrix_rows:
            matrix_df = pd.DataFrame(matrix_rows)
            st.dataframe(
                matrix_df[
                    ["requirement_id", "requirement", "suggested_owner", "draft_response", "source_ids", "review_status", "notes"]
                ],
                use_container_width=True,
                hide_index=True,
            )
            st.download_button(
                "Export response matrix as CSV",
                matrix_df.to_csv(index=False).encode("utf-8"),
                file_name=f"{req_id.lower()}_response_matrix.csv",
                mime="text/csv",
                key=f"download_ai_matrix_{req_id}",
            )
        else:
            st.info("No requirement-level response matrix was returned.")

        overall_draft = result.get("draft_response", "")
        if isinstance(overall_draft, (dict, list)):
            overall_draft = json.dumps(overall_draft, ensure_ascii=False, indent=2)
        st.write("**Overall draft / cover response — NOT APPROVED**")
        st.text_area("Draft", value=str(overall_draft or ""), height=180, key=f"draft_{req_id}")
        st.info("The app intentionally does not provide an autonomous Send/Submit action.")



def response_matrix_tab() -> None:
    st.subheader("5 · Requirement-by-requirement response matrix")
    st.caption(
        "This is the controlled working matrix for the uploaded RFP/questionnaire. "
        "AI drafts are not approvals; source evidence, accountable owner and review status remain visible for each requirement."
    )
    requests = st.session_state["requests"]
    if not requests:
        st.info("Add a request first.")
    ...
    req_id = st.selectbox("Select request", [r["request_id"] for r in requests], key="matrix_req")
    req = next(r for r in requests if r["request_id"] == req_id)
    result = req.get("ai_result") or {}
    if not result:
        st.info("Run AI Review for this request to generate the response matrix.")
        return

    rows = normalize_response_matrix(result, req)
    if not rows:
        st.info("No response matrix is available for this request.")
        return

    rdf = pd.DataFrame(rows)
    status_counts = rdf["review_status"].value_counts().to_dict() if "review_status" in rdf.columns else {}
    c1, c2, c3, c4 = st.columns(4)
    c1.metric("Requirements", len(rdf))
    c2.metric("Ready for human QA", int(status_counts.get("Ready for human QA", 0)))
    c3.metric("SME approval required", int(status_counts.get("SME approval required", 0)))
    c4.metric(
        "Blocked / evidence missing",
        int(status_counts.get("Blocked", 0)) + int(status_counts.get("Evidence missing", 0)),
    )

    st.dataframe(
        rdf[
            ["requirement_id", "requirement", "category", "suggested_owner", "draft_response", "source_ids", "review_status", "notes"]
        ],
        use_container_width=True,
        hide_index=True,
    )
    st.caption(
        "Source IDs are evidence references, not automatic approval. Rows marked SME approval required, Evidence missing or Blocked "
        "must remain under accountable human review before release."
    )
    st.download_button(
        "Export RFP response matrix as CSV",
        rdf.to_csv(index=False).encode("utf-8"),
        file_name=f"{req_id.lower()}_rfp_response_matrix.csv",
        mime="text/csv",
        key=f"download_matrix_tab_{req_id}",
    )



def qa_tab() -> None:
    st.subheader("6 · Second-read QA and human release gate")
    requests = st.session_state["requests"]
    if not requests:
        st.info("Add a request first.")
        return
...
def architecture_tab() -> None:
    st.subheader("7 · Why the controls exist")
    st.markdown(
        """
...
- **Canonical-source library** prevents plausible-but-unverified answers from becoming customer-facing commitments and treats the stored source date explicitly as the **next review date** for deadline freshness checks.
- **AI retrieval and drafting** accelerates research and first drafts, but only from Approved material.
- **RFP response matrix** preserves requirement-level traceability across draft answer, source IDs, accountable owner and review status instead of collapsing the package into one free-text answer.
...
def main() -> None:
    ...
    st.title(APP_TITLE)
    st.write("A fictional, human-in-the-loop proof of concept for structured intake, document/requirements ingestion, risk-based routing, canonical-source governance, requirement-level response drafting, deadline control and release QA.")
    ...
    tabs = st.tabs(["Dashboard", "Request Intake", "Requirements", "Source Library", "AI Review", "Response Matrix", "QA & Release", "Architecture"])
    ...
    with tabs[5]: response_matrix_tab()
    with tabs[6]: qa_tab()
    with tabs[7]: architecture_tab()
